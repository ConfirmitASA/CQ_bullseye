...
var BullsEye = /*#__PURE__*/function () {
  function BullsEye(question) {
    ...
    var translations = arguments.length > 11 ? arguments[11] : undefined;
  ...
  _createClass(BullsEye, [{
    ...
    value: function putItemOnTarget(item, target, degreeOffset) {
      ...
      var originX = $('[data-group-name="' + group + '"]').position().left;
      var originY = $('[data-group-name="' + group + '"]').position().top;
    ...
    value: function drawCircle(x, y, color, elemId, radius, rank) {
      ...
      if (elemId == "target_" + (this.centerIsActive ? this.question.scales[0].code : "")) {
        $(document.createElementNS('http' + '://www.w3.org/2000/svg', 'text')).attr('x', "50%").attr('y', "50%").attr('text-anchor', "middle").attr('dy', '.3em').attr('class', 'metext').attr('height', '30').attr('width', '60').attr('fill', this.centerTextColor).attr('font-size', radius / 2).text(this.getTranslatedCenterText()).appendTo($svg);
      }
    }
  }, {
    key: "getTranslatedCenterText",
    value: function getTranslatedCenterText() {
    ...
    key: "getTranslatedNumberOfRequired",
    value: function getTranslatedNumberOfRequired() {
